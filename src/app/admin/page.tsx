    ...
    <AdminShell profile={profile}>
      ...
      <section className="mt-12 rounded-lg border border-border bg-background p-6 sm:p-8">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <QuickLink href={`${brand.siteUrl}/admin/leads`} label="Leads" description="View and export captured leads" />
          <QuickLink href={`${brand.siteUrl}/admin/quizzes`} label="Quizzes" description="Build and manage assessments" />
          <QuickLink href={`${brand.siteUrl}/admin/analytics`} label="Analytics" description="Funnel metrics and trend data" />
          <QuickLink href={`${brand.siteUrl}/admin/destinations`} label="Destinations" description="HubSpot and webhook delivery" />
          <QuickLink href={`${brand.siteUrl}/admin/prompts`} label="AI Prompts" description="Edit the narrative prompt template" />
          <QuickLink href={brand.siteUrl} label="View live site" description="See the assessment portal as a prospect would" external />
        </div>
      </section>
    </AdminShell>
  );
